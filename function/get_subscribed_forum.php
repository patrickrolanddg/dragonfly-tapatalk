...
{
    global $board_config, $db, $user, $auth, $mobiquo_config, $phpbb_home;
    ...
    $forum_list = array();
    if ($board_config['allow_forum_notify'])
    {
        ...
        {
            $tracking_topics = (isset($_COOKIE[$board_config['cookie_name'] . '_track'])) ? ((STRIP) ? stripslashes($_COOKIE[$board_config['cookie_name'] . '_track']) : $_COOKIE[$config['cookie_name'] . '_track']) : '';
        ...
        while ($row = $db->sql_fetchrow($result))
        {
            ...
            {
                $forum_check = (isset($tracking_topics['f'][$forum_id])) ? (int) (base_convert($tracking_topics['f'][$forum_id], 36, 10) + $board_config['board_startdate']) : $user->data['user_lastmark'];
            }
...
//            );
            ...
            else if (file_exists(MOBPATH."forum_icons/$forum_id.jpg"))
            {
                $logo_url = MOBPATH."forum_icons/$forum_id.jpg";
            }
            else if (file_exists(MOBPATH."forum_icons/default.png"))
            {
                $logo_url = MOBPATH."forum_icons/default.png";
            }
            else if ($row['forum_image'])
            {
                $logo_url = MOBPATH."forums_icons/{$row['forum_image']}";
            }
